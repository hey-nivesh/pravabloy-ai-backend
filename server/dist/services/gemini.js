...
const DEFAULT_VOICE_POOL = ['Puck', 'Kore'];
...
const OPENER_FALLBACKS = {
    mock_interview: 'You are now on a live video interview. Please greet me, introduce yourself briefly as the interviewer, and ask me your first question.',
    executive: 'The meeting has begun. Please greet me as your colleague and open the negotiation with your first position statement.',
    formal: 'The session is starting. Please greet me professionally and frame the first topic you want to discuss.',
    casual: 'We\'re live! Please greet me warmly and ask me a casual opener to get the conversation going.',
};
...
Where natural, steer the conversation to give the learner an opportunity to use the vocabulary list above. Keep a mental note of notable grammar mistakes or filler-word usage, but do NOT correct them live. Correction is handled separately at the end of the session. Keep your spoken responses short (1-2 sentences) to encourage the user to speak more.
  `.trim();
    ...
    const connectPromise = ai.live.connect({
        ...
        callbacks: {
            onopen: () => {
                ...
                const openerInstruction = params.scenarioPrompt ||
    ...
    const sessionTimeout = setTimeout(() => {
        console.warn(`[GeminiLiveSession] Session ${params.sessionId} exceeded ${MAX_SESSION_DURATION_MS / 60000} minutes. Force closing.`);
        handleClose();
    ...
    return {
        ...
        sendTextEvent(event) {
            if (isClosed || !isConnected || !sessionObject)
                return;
            if (event.text) {
