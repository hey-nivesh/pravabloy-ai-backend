...
exports.MAX_CONCURRENT_LIVE_SESSIONS = void 0;
...
exports.MAX_CONCURRENT_LIVE_SESSIONS = parseInt(process.env.MAX_CONCURRENT_LIVE_SESSIONS || '3', 10);
...
function acquireSession() {
    if (activeSessions >= exports.MAX_CONCURRENT_LIVE_SESSIONS) {
        throw new Error(`Server is at live session capacity (${exports.MAX_CONCURRENT_LIVE_SESSIONS} concurrent sessions). Please try again in a moment.`);
    }
    activeSessions++;
    console.log(`[SessionPool] Slot acquired. Active sessions: ${activeSessions}/${exports.MAX_CONCURRENT_LIVE_SESSIONS}`);
    let released = false;
    return () => {
        if (!released) {
            released = true;
            activeSessions = Math.max(0, activeSessions - 1);
            console.log(`[SessionPool] Slot released. Active sessions: ${activeSessions}/${exports.MAX_CONCURRENT_LIVE_SESSIONS}`);
        }
    };
}
