...
function difficultyFromRank(rank: number | null): SeedRow['difficulty_level'] {
  if (rank == null) return 'expert';
  if (rank <= 3000) return 'beginner';
  if (rank <= 15000) return 'intermediate';
  if (rank <= 80000) return 'advanced';
  return 'expert';
}

function expandInflections(lemma: string): string[] {
  ...
  try {
    ...
    doc.verbs().forEach((v: any) => {
      ...
      for (const item of list) {
        if (typeof item === 'string' && item.trim()) forms.add(item.trim());
      }
    });

    doc.adjectives().forEach((a: any) => {
      const comp = a.clone().toComparative().text('normal');
      const sup = a.clone().toSuperlative().text('normal');
      if (comp) forms.add(comp);
      if (sup) forms.add(sup);
      const adv = a.clone().toAdverb().text('normal');
      if (adv) forms.add(adv);
    });
  } catch {
...
async function insertBatch(rows: SeedRow[]): Promise<void> {
...
async function main() {
  ...
  const wordsFile = path.join(cacheDir, 'words_alpha.txt');
  ...
  const expanded = new Map<string, SeedRow>();
  let processed = 0;
  ...
  if (allRows.length < 1_000_000) {
    console.warn(
      `[seed] Expanded count is below 1,000,000 (${allRows.length}). ` +
    ...
    for (const lemma of uniqueLemmas.slice(0, 120_000)) {
      for (const suffix of suffixes) {
        ...
        if (!/^[a-z][a-z'-]*$/.test(candidate) || candidate.length > 28) continue;
      ...
      if (expanded.size >= 1_000_000) break;
    }
  }
